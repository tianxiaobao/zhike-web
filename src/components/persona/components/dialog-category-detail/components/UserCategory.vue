<template>
    <chart-container>
        <div slot="header">
            <h2>{{ queryConditions.category }}客户三级品类偏好</h2>
        </div>
        ...
        onConditionChange () {
            ...
            case types.CUSTOMER_FLOW:
                if(queryConditions) {
                    ...
                    const getter = this.getter = DataGetter.createAjaxGetter(getThirdByUser, {
            ...
            case types.VIP:
                if(queryConditions) {
                    const getter = this.getter = DataGetter.createAjaxGetter(getMemberThirdByUser, {
            ...
            case types.CUSTOM:
                if(queryConditions) {
                    const getter = this.getter = DataGetter.createAjaxGetter(getCustomThirdByUser, {
