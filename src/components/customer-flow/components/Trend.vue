...
import { getTrendHistory, getTrendAverage } from '@/apis/customerFlow';
    ...
    methods: {
        ...
        queryData () {
            ...
            if (queryConditions.time === 0) {
                Promise.all([
                    ...
                    this.updateTrendToday(queryDelay, '今日', 0);
        ...
        queryPicked () {
            ...
            getTrendHistory({
                ...
                this.$nextTick(() => this.showChart = true);
                if (todayIndex !== -1) {
                    this.updateTrendToday(queryDelay, null, todayIndex);
                }
            }).catch(this.handleQueryError);
        },
        updateTrendToday (timeout, name, idx) {
            this.timer = setTimeout(() => {
                ...
                getTrendHistory({
                    ...
                    this.token = null;
                    this.tmp[idx] = this.adaptSingleDayData(dataObj.message.DataResult, name);
                    this.data = {
                        xLabels: this.tmp[0].xLabels,
                        series: this.tmp.map(data => data.series[0])
                    };
                    this.$nextTick(() => this.showChart = true);
                    this.updateTrendToday(timeout, name, idx);
                }).catch(this.handleQueryError);
            }, timeout);
        },
        onConditionsChange () {
